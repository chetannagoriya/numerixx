...
export default function Bracelets(){return <main><SiteHeader/>
  <section className="shop-hero"><div><p className="eyebrow"><Sparkles size={14}/> Crystal bracelets</p><h1>Wear Your Numbers.<br/><em>Carry Your Energy.</em></h1><p>Handcrafted crystal bracelets made with authentic natural stones to align your energy, attract positivity, and support your numerology journey.</p><Link className="button gold" href="#bracelet-collection">Shop the collection <ArrowRight size={16}/></Link></div><img src="/assets/bracelets-hero.jpg" alt="Tiger's eye and green aventurine crystal bracelets"/></section>
  <section className="shop-benefits">{benefits.map(([Icon,title])=><div key={String(title)}><Icon/><span>{String(title)}</span></div>)}</section>
  <section className="shop-stats"><div><strong>200+</strong><span>Products curated with care</span></div><div><strong>8+</strong><span>Categories for every journey</span></div><div><strong>5,000+</strong><span>Happy customers</span></div><div><strong>7 Days</strong><span>Easy returns</span></div></section>
  <section className="shop-collection" id="bracelet-collection"><div className="section-heading"><p className="eyebrow centered"><Sparkles size={14}/> Shop by energy</p><h2>Crystal Bracelet Collection</h2><p>Choose the stone and intention that feels aligned with your journey.</p></div><div className="product-grid">{products.map(([title,copy,price],i)=><article className="product-card" key={title}><div className="product-image"><img src={`/assets/bracelet-${i+1}.jpg`} alt={title}/>{i===0&&<span>Best seller</span>}{i===7&&<span>Custom</span>}</div><div className="product-details"><h3>{title}</h3><p>{copy}</p><strong>{price}</strong><Link href="/contact"><ShoppingBag size={15}/> Enquire now</Link></div></article>)}</div></section>
  <section className="shop-promise">{[[Gem,'100% Authentic Natural Stones','Sourced from trusted suppliers worldwide.'],[Sun,'Energized & Blessed','Each bracelet is cleansed and energized for you.'],[ShieldCheck,'Premium Quality','Finest grade stones with attention to detail.'],[RotateCcw,'Easy Guidance','Personal support in choosing your bracelet.']].map(([Icon,title,copy])=><div key={String(title)}><Icon/><p><strong>{String(title)}</strong><span>{String(copy)}</span></p></div>)}</section>
