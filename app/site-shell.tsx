...
export function SiteHeader(){return <header className="site-header"><Link className="brand" href="/" aria-label="Numerixx home"><img className="brand-logo" src="/assets/numerixx-logo.png" alt=""/><span><img className="brand-wordmark" src="/assets/numerixx-wordmark.png" alt="Numerixx"/><small>Decode · Understand · Transform</small></span></Link><nav aria-label="Primary navigation">{links.map(([href,label])=><Link href={href} key={href}>{label}</Link>)}</nav><Link className="button button-small" href="/contact">Book appointment <ArrowUpRight size={15}/></Link><Link className="menu" href="/contact" aria-label="Open contact page"><Menu/></Link></header>}
